...
# 我自己写的CarDamage类(多分支，如果是单分支 直接使用COCO官方对应的API就行了)
class CarDamage:
    ...
    #============添加的两个额外信息==============
    def extar_index(self, remove_images_without_annotations=True):
        ...
        if remove_images_without_annotations:
            ...
            for img_id in self.ids:
                ...
                anno = self.loadAnns(ann_ids)
                if has_valid_annotation(anno):
                    ids.append(img_id)
            self.ids = ids

        # 添加损伤总类别
        self.categories = {cat['id']: cat['name'] for cat in self.cats.values()}
        # 添加零件总类别
        self.components = {cat['id']: cat['name'] for cat in self.comps.values()}
        ...
        self.json_component_id_to_contiguous_id = {
            v: i + 1 for i, v in enumerate(self.getCompIds())
        }
